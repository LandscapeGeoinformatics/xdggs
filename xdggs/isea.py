...
class ISEAIndex(DGGSIndex):
    ...
    def from_variables(
        ...
        *,
        ...
        cellids = None
        x_batch = int(np.ceil(x.shape[0] / step))
        ...
        if (method.lower() == 'nearestpoint'):
            ...
            with pymp.Parallel(mp) as p:
                for b in tqdm(p.range(len(job))):
                    ...
                    del y_trunk
                    working_dir = tempfile.mkdtemp()
                    dggs = DGGRIDv7(executable=executable, working_dir=working_dir)
                    result = dggs.grid_cell_centroids_for_extent(grid.upper(), resolution, clip_geom=truck_df.geometry.values[0])
